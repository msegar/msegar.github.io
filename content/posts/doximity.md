...
The most valuable feature is custom template creation. I have since taken this further and use a separate template for [precharting](/blog/posts/doximity_precharting.html), reading the chart aloud before clinic and letting Scribe structure it. iPhone dictation mangles medical terms on the way in, so I now [dictate with a local Whisper model](/blog/posts/local_whisper_dictation.html) instead. Here's the cardiology new patient encounter template I developed:
